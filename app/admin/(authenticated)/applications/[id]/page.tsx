...
export default function ApplicationDetailPage({ params }: PageProps) {
    ...
    const handleDownloadResume = async () => {
        const resumeUrl = application?.resume?.url || application?.user?.resume?.url;
        const filename = application?.resume?.filename || application?.user?.resume?.filename || 'resume.pdf';
                        ...
                        <div className="flex flex-wrap gap-3 mb-6">
                            ...
                            {(application.resume?.url || application.user?.resume?.url) && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors text-sm font-medium disabled:opacity-50"
                                >
                                    {downloadingResume ? (
